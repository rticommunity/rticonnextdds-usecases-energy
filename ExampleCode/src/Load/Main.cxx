...
void PrintHelp()
{
    ...
    cout << "    --config [path]"
        ...
        << "                               "
        << "Default is Load.ini." << endl;
}

int main(int argc, char* argv[])
{
    int domainId = 0;
    std::string configFile = "Load.ini";
    ...
    try {
        ...
        string entityName("Load-" + config.Get("IED", "DeviceID", "SimLoad"));

        auto&& load = new Load(domainId, entityName, config);

        std::thread executeThread(&Load::Execute, load);

        cout << "Load Unit Operating. Press Enter to quit." << endl;
