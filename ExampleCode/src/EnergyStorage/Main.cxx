...
void PrintHelp()
{
    ...
    cout << "    --config [path]"
        ...
        << "                               "
        << "Default is EnergyStorage.ini." << endl;
}
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        string entityName("ES-" + config.Get("IED", "DeviceID", "SimBattery"));

        auto&& es = new EnergyStorage(domainId, entityName, config);

        std::thread executeThread(&EnergyStorage::Execute, es);

        cout << "Energy Storage Unit Operating. Press Enter to quit." << endl;
        ...
        cout << "Energy Storage Unit Shut Down." << endl;
    }
