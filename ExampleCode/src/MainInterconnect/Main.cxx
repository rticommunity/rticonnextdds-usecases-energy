...
void PrintHelp()
{
    ...
    cout << "    --config [path]"
        ...
        << "                               "
        << "Default is MainInterconnect.ini." << endl;
}
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        string entityName("Interconnect-" + config.Get("IED", "DeviceID", "SampleInterconnect"));

        auto&& interconnect = new MainInterconnect(domainId, entityName, config);

        std::thread executeThread(&MainInterconnect::Execute, interconnect);

        cout << "Main Interconnect Unit Operating. Press Enter to quit." << endl;

        cin.get();

        interconnect->StopIED();
        executeThread.join();

        cout << "Main Interconnect Unit Shut Down." << endl;
    }
