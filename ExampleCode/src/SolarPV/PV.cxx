        ...
        dds::pub::DataWriter<Energy::Ops::Status_Device> WriterStatus_Device)
{
    ...
    Energy::Ops::Status_Device sampleStatus_Device(
            DeviceID,
    ...
    case Energy::Enums::DeviceControl::DISCONNECT:
        connected = false;
        sampleStatus_Device.ConnectionStatus(
                Energy::Enums::ConnectionStatus::DISCONNECTED);
        sampleStatus_Device.OperationStatus(
                Energy::Enums::OperationStatus::DISABLED_READY);
        break;
    default:
        break;
    }
...
float GetMeasurement()
{
    ...
    // case we're just going to calculate the power based on curtailment and
    // irradiance
...
/* ContinuousWriter
 * In this example we are using a function in a seperate thread to continously
...
{
    Energy::Ops::Meas_NodePower sampleMeas_NodePower(
            DeviceID,
            SimMeasurement,
            NodeID);
...
void publisher_main(int domain_id)
{
    ...
    dds::topic::Topic<Energy::Ops::Meas_NodePower> TopicMeas_NodePower(
            participant,
            "Meas_NodePower");
    dds::topic::Topic<Energy::Ops::Info_Resource> TopicInfo_Resource(
            participant,
            "Info_Resource");
    dds::topic::Topic<Energy::Ops::Status_Device> TopicStatus_Device(
            participant,
            "Status_Device");
    ...
    dds::topic::Topic<Energy::Common::CNTL_Single_float32> TopicControl_Power(
            participant,
            "Control_Power");
    dds::topic::Topic<Energy::Common::CNTL_Single_float32>
            TopicControl_Irradiance(participant, "Control_Irradiance");
    ...
    dds::pub::qos::DataWriterQos qos_control_load =
            dds::core::QosProvider::Default().datawriter_qos(
    ...
    dds::sub::cond::QueryCondition QueryConditionControl_Device(
            ...
            [&ReaderControl_Device,
             &WriterStatus_Device](dds::core::cond::Condition condition) {
    ...
    // Query Condition for power curtailment. This is basic functionality for a
    // PV system
    ...
    dds::sub::cond::QueryCondition QueryConditionControl_Irradiance(
            ...
            [&ReaderControl_Irradiance](dds::core::cond::Condition condition) {
                auto condition_as_qc = dds::core::polymorphic_cast<
                        dds::sub::cond::QueryCondition>(condition);
                auto samples = ReaderControl_Irradiance.select()
                                       .condition(condition_as_qc)
                                       .read();
                for (auto sample : samples) {
                    // All valid samples will be processed and set the global
    ...
    Energy::Common::CNTL_Single_float32 sampleControl_Power(
            ...
            DeviceID,
            8.0);
    Energy::Common::CNTL_Single_float32 sampleControl_Irradiance(
            DeviceID,
            DeviceID,
            700.0);
    ...
    dds::core::cond::StatusCondition StatusConditionControl_Device(
            ReaderControl_Device);
    dds::core::cond::StatusCondition StatusConditionControl_Power(
            ReaderControl_Power);
    ...
    StatusConditionControl_Device->handler([&ReaderControl_Device,
                                            &WriterStatus_Device](
                                                   dds::core::cond::Condition
                                                           condition) {
        dds::core::status::StatusMask status_mask =
                ReaderControl_Device.status_changes();
    ...
    StatusConditionControl_Power->handler([&ReaderControl_Power](
                                                  dds::core::cond::Condition
                                                          condition) {
        dds::core::status::StatusMask status_mask =
                ReaderControl_Power.status_changes();
        if ((status_mask & dds::core::status::StatusMask::liveliness_changed())
                    .any()) {
            auto condition_as_qc =
                    dds::core::polymorphic_cast<dds::sub::cond::QueryCondition>(
