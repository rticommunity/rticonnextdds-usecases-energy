...
void PrintHelp()
{
    ...
    cout << "    --config [path]"
        ...
        << "                               "
        << "Default is SolarPV.ini." << endl;
}

int main(int argc, char* argv[])
{
    int domainId = 0;
    std::string configFile = "SolarPV.ini";
    ...
    try {
        ...
        string entityName("PV-" + config.Get("IED", "DeviceID", "SimPV"));

        auto&& pv = new SolarPV(domainId, entityName, config);

        std::thread executeThread(&SolarPV::Execute, pv);

        cout << "Solar PV Unit Operating. Press Enter to quit." << endl;
        ...
        cout << "Solar PV Unit Shut Down." << endl;
    }
